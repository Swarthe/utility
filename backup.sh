...
# add mention of config file at ~/.config/utilitysh/backup.conf for non-root user, probably in help text
...
if [ -z $skip ]; then
    ...
    if [ -z $skip_free ]; then
        ...
        -gt $(df --output=avail -k "$targ" | tail -n 1) ]; then
            ask "Target '$targ' has insufficient free space. Proceed anyway?"
        fi
    fi
